...
与文本测试（test_engine.py）同构：并发度 = 同时运行的 case 数，
每个 case 循环处理分到的图片——把图片（base64 多模态消息）发给
被测模型，要求其理解图片内容并按指定字数生成一篇文章。

图片池：启动时扫描用户选择的目录（按文件名排序保证稳定），
总量 = 迭代次数 × 并发度，全局 round-robin 循环取图（目录图片
...
def scan_image_dir(image_dir: str) -> list[str]:
    """扫描目录下的全部图片文件（按文件名排序，保证池分配稳定）。
...
def build_image_prompt(output_tokens: int) -> str:
    """图形测试 prompt：理解图片 + 按输出长度生成文章。

    output_tokens 为目标输出 token 数，按 ~1.7 字/token 折算为字数
    写入指令（与文本测试口径一致，模型对「字数」遵循更好）。
    """
    n_chars = max(20, round(output_tokens * 1.7))
    return (f"请仔细观察这张图片，理解图片的内容与细节，"
            f"然后写一篇约{n_chars}字的文章。文章应围绕图片中的"
            f"主体与场景展开，写作时请严格遵循该字数要求，"
            f"直接输出文章正文。")


def _read_image_data_url(path: str) -> str:
    """读图片文件并编码为 data URL（base64）。"""
...
# 单个 case 的状态（与文本测试 CaseState 同构，名词 → 图片文件名）
@dataclass
class CaseState:
    case_id: int
    ...
    images: list = field(default_factory=list)          # 本 thread 分到的图片文件名列表
    ...
    def add_error(self, phase: str, error: str, *, loop: int = 0,
                  image: str = "", duration: float = 0.0) -> None:
        """记录一次错误明细（供前端悬浮框展示）。"""
        self.errors += 1
        self.last_error = error
        self.error_records.append({
            "ts": round(time.time(), 1),   # 发生时间（epoch 秒）
            "phase": phase,                # 阶段：生成文章/读取图片/内部异常
            "loop": loop,                  # 第几轮（1-based）
            "image": image,                # 出错时正在处理的图片文件名
            "duration": round(duration, 1),  # 该次调用耗时（秒）
            "error": error,                # 错误信息（含异常类型）
        })

    def begin_qa(self, phase: str, *, loop: int = 0, image: str = "",
                 question: str = "") -> dict:
        """开始一次调用：追加记录并返回引用（流式期间更新其 partial 字段）。"""
        qa = {
            "phase": phase,           # 生成文章
            "loop": loop,             # 第几轮
            "image": image,           # 本次处理的图片文件名（仅存名不存图，控制体积）
            "question": question,     # 发给 LLM 的文本 prompt（图形测试 prompt 很短，全文存储）
            "answer": "",             # 完成后的完整回答
...
class ImageTestEngine:
    ...
    def __init__(self):
        ...
        self._image_pool: list[str] = []  # 全局图片池（各 thread 取不重叠分片，存绝对路径）
        self.task_name: str = ""          # 本轮测试任务名（workspace 目录名）

    # ---------- 启动 / 停止 ----------

    def start(self, model_id: str, image_dir: str, image_count: int,
        ...
        # 扫描图片目录（不存在/无权限时 scandir 抛 OSError）
        try:
            image_names = scan_image_dir(image_dir)
        except OSError as e:
            return {"success": False,
                    "error": f"无法读取图片目录：{e}"}
        if not image_names:
            return {"success": False,
                    "error": f"目录中没有图片文件（支持 {'/'.join(sorted(IMAGE_EXTS))}）"}
        ...
        self.params = {
            ...
            # 目录内图片总数（报告 Profile 展示；池不够用时前端可提示重复使用）
            "image_pool_size": len(image_names),
        }
        self.test_id = f"img-{int(time.time()*1000)}"
        self.task_name = workspace.new_task_name("图形测试")
        self.cases = [CaseState(case_id=i + 1) for i in range(concurrency)]
        for c in self.cases:
            c.status = "queued"
        # 全局图片池：总量 = 迭代次数 × 并发度，round-robin 循环取图
        #   （目录图片不够用时重复使用），各 thread 取不重叠分片
        total = image_count * concurrency
        self._image_pool = [os.path.join(image_dir, image_names[i % len(image_names)])
        ...
        #   造成 prefill 风暴（图片 prefill 开销远大于文本，错峰更有意义）。
        ...
        applog.log("test", f"启动图形测试 {self.task_name} "
    ...
    def stop(self) -> dict:
        ...
        self._tasks = []
        applog.log("test", f"用户停止图形测试 {self.task_name}")
        return {"success": True}
    ...
    async def _wait_all(self):
        ...
        if self.task_name:
            try:
                ...
                applog.log("test", f"图形测试结束 {self.task_name} status={self.status} "
    ...
    async def _run_case(self, case: CaseState, start_delay: float = 0.0):
        ...
        try:
            # 第一步：从全局图片池取本 thread 的分片
            #   池在 start() 时按 迭代次数 × 并发度 round-robin 生成：
            #   目录图片够用时各 thread 分片不重叠，不够时循环重复
            n = self.params["image_count"]
            base = (case.case_id - 1) * n
            image_paths = self._image_pool[base:base + n]
            case.images = [os.path.basename(p) for p in image_paths]
            case.total_loops = len(image_paths)
            ...
            for i, path in enumerate(image_paths):
                ...
                case.current_image = image_name
                case.completed_loops = i
                prompt = build_image_prompt(self.params["article_length"])
                # 多模态消息：图片（data URL）+ 文本指令（OpenAI 标准格式）
                try:
                    # 本地读图 + base64 编码放线程池，避免大图高并发时
                    # 阻塞事件循环（SSE 推送/指标轮询都跑在同一循环上）
                    data_url = await asyncio.to_thread(_read_image_data_url, path)
                except OSError as e:
                    case.calls_done += 1
                    case.add_error("读取图片", f"无法读取图片文件: {e}",
                                   loop=i + 1, image=image_name)
                    continue
                messages = [{
                    "role": "user",
                    "content": [
                ...
                qa = case.begin_qa("生成文章", loop=i + 1, image=image_name,
                                   question=prompt)
                ...
                r = await chat_completion_stream(
                    ...
                    timeout=600.0,
                    on_chunk=_on_chunk,
                    # 每请求精确输入 token（图片 token 数只能由服务端
